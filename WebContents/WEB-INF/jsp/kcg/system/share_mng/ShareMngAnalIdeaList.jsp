			...
			<div class="dataTables_wrapper" id="vueapp">
				<template>
					...
					<div class="dataTables_filter">
						...
						</select> <input type="search" placeholder="" style="width: 120px;"
							id="search_val" v-model="search_val" @keyup.enter="getList(true)">
						<button type="button" class="btn btn-blue btn-icon icon-left"
							style="margin-left: 5px;" @click="getList(true)">
					...
					<table class="table table-bordered datatable dataTable"
						...
						<thead>
							<tr class="replace-inputs">
								...
								<th style="width: 50%;" class="center sorting"
									@click="sortList(event.target)" sort_target="sbjt">제목</th>
								<th style="width: 20%;" class="center sorting"
									@click="sortList(event.target)" sort_target="reg_user_name">작성자</th>
								<th style="width: 15%;" class="center sorting"
									@click="sortList(event.target)" sort_target="reg_dt">작성일</th>
								<th style="width: 10%;" class="center sorting"
									@click="sortList(event.target)" sort_target="read_cnt">조회</th>
							</tr>
						</thead>
						<tbody id="tbody_list">
							<tr v-for="item in dataList" @click="gotoDtl(item.idx)"
	...
	var vueapp = new Vue({
		...
		methods : {
			getList : function(isInit) {
				...
				cf_ajax("/system/share_mng/analIdea/getList", params,
						this.getListCB);
			},
			getListCB : function(data) {
				this.dataList = data.list;
				for (var i = 0; i < this.dataList.length; i++) {
					this.dataList[i].read_cnt = this.dataList[i].read_cnt
							.numformat();
				}
