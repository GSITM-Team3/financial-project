               ...
               <div class="panel panel-primary" data-collapsed="0">
                  <div class="panel-body">
                     <form class="form-horizontal form-groups-bordered">
                        ...
                        <div class="form-group">
                           <label for="ctnt" class="sys_label_01 control-label">분석 아이디어</label>
                        ...
                        <div class="form-group">
                           <div class="col-sm-offset-2 col-sm-5">
                              <button type="button" class="btn btn-green btn-icon btn-small" v-if="info.save_mode === 'insert'" @click="save">
                                 저장
                                 <i class="entypo-check"></i>
                              </button>
                              <button type="button" class="btn btn-green btn-icon btn-small" v-else @click="save">
                                                                            수정
                                           <i class="entypo-check"></i>
                                        </button>
                              <button type="button" id="btn_delete" class="btn btn-red btn-icon btn-small" v-if="info.save_mode !== 'insert'" @click="delInfo">
                                 삭제
...
var vueapp = new Vue({
   ...
   mounted : function(){
      if(!cf_isEmpty(this.info.idx)){
         ...
         // 초기 렌더링 시점에서 저장 모드가 insert일 경우 h2 태그 텍스트를 설정
            if (this.info.save_mode === 'insert') {
                $(".notice-title").text("공지사항 > 작성하기");
            }
      }
   },
   methods : {
      getInfo : function(){
         cf_ajax("/system/share_mng/analIdea/getInfo", this.info, this.getInfoCB);
      },
      getInfoCB : function(data){
         this.info.save_mode = "update";
         this.info = data;
         $("#ctnt").summernote("code", this.info.ctnt);
         $(".notice-title").text("공지사항 > 수정하기");
      },
      save : function(){
         ...
         // 7월 10일 15시 40분 : 추가
            this.info.save_mode = this.info.idx ? "update" : "insert";
         
         cf_ajaxWithFiles("/system/share_mng/analIdea/save", fileList, this.info, this.saveCB);
      },
      saveCB : function(data){
         //this.info.idx = data.idx;
         //this.getInfo();
         cf_movePage('/system/share_mng/analIdea/list?fromDtl=Y'); // 추가
      },
      delInfo : function(){
         if(!confirm("삭제하시겠습니까?")) return;
         cf_ajax("/system/share_mng/analIdea/delete", this.info, this.delInfoCB);
      },
      delInfoCB : function(data){
         this.gotoList();
      },
      gotoList : function(){
         cf_movePage('/system/share_mng/analIdea/list?fromDtl=Y');
      },
   }
});
</script>
