...
<body>
	...
	<!-- 출력 로그 modal -->
	<!-- The Modal -->
	<div class="modal fade stdOutModal" role="dialog">
		<!-- Modal content -->
		<div class="modal-content resultLog">
			<h2>출력로그<span class="close" data-dismiss="modal">&times;</span></h2>
	...
	<!-- 에러 로그 modal -->
	<!-- The Modal -->
	<div class="modal fade stdErrModal" role="dialog">
		<!-- Modal content -->
		<div class="modal-content resultLog">
			<h2>에러로그<span class="close" data-dismiss="modal">&times;</span></h2>
			<div class="modal-body">
	            ...
	            if(status == "success") {
	                ...
	                if(list.length > 0){
	                	...
	                	$.each(list, function(i, value){
							...
							str += '<td>' + fn_isEmpty(value.tableKoreanNm) + '</td>'
							str += '<td>' + value.tableEngNm + '</td>'
							...
							str += '<td class="log"><a href="javascript:void(0);" onclick="fn_stdoutModal(\'' + value.plannedDt
									+ '\', \'' + value.cntcMthdCode
									+ '\', \'' + value.cnncManageNo
									+ '\', \'' + value.tableEngNm + '\');"><strong>' + fn_isEmpty(value.stdout) + '</strong></a></td>'
							str += '<td class="log"><a href="javascript:void(0);" onclick="fn_stderrModal(\'' + value.plannedDt
									+ '\', \'' + value.cntcMthdCode
									+ '\', \'' + value.cnncManageNo
									+ '\', \'' + value.tableEngNm + '\');"><strong>' + fn_isEmpty(value.stderr) + '</strong></a></td>'
							str += '</tr>'
	                	});
	...
	function fn_stdoutModal(plannedDt, cntcMthdCode, cnncManageNo, tableEngNm) {
		$.ajax({
	        ...
	        success : function(returnData, status) {
				...
				var stdout = '<p>' + fn_isEmpty(returnData.stdout) + '</p>';
	...
	function fn_stderrModal(plannedDt, cntcMthdCode, cnncManageNo, tableEngNm) {
		$.ajax({
	        ...
	        success : function(returnData, status) {
				...
				var stderr = '<p>' + fn_isEmpty(returnData.stderr) + '</p>';
