...
function init() {
	...
	Module.getNavigation().setPadding(70, 130);
...
function initEvent(_canvas) {
	...
	/*canvas.addEventListener("Fire_click",function(e) {
		if(WDEVT_FLAG){ //날씨 팝업창 떠있으면
	...
	canvas.addEventListener("Fire_EventSelectedObject", function(e) {
		...
		if(e.objKey.indexOf('wth') != -1){ //날씨 아이콘 클릭 이벤트
			weatherIcoEvtHd(e);
			WDEVT_FLAG = true;
		}
		...
		/*
		//영역검색 - 연안사고
		if(e.objKey.indexOf('search_area_accident_') != -1){
			
			setHighlightIcon("SEARCH_AREA_ACCIDENT_MARKER",e.objKey);
			
		}
		//영역검색 - 선박사고
		if(e.objKey.indexOf('search_area_ship_accident_') != -1){
			
			setHighlightIcon("SEARCH_AREA_SHIP_ACCIDENT_MARKER",e.objKey);
			
		}
		//영역검색 - 주제도
		if(e.objKey.indexOf('search-area-') != -1){
			
			setHighlightIcon("SEARCH_AREA_"+evt.objKey.split('-')[2]+"_MARKER",e.objKey);
